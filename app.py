...
def chiave_notifica(notifica):
  """Identificatore stabile per non riproporre notifiche già lette."""
  return json.dumps(notifica, sort_keys=True, ensure_ascii=False, default=str)
...
def parse_data_evento(d_str):
  ...
  if isinstance(d_str, date):
    return datetime.combine(d_str, datetime.min.time())
...
def data_display_con_orario(data_input, orario):
  """Normalizza la data mostrata e conserva l'orario senza nuove colonne DB."""
  data_normalizzata = parse_data_evento(data_input).strftime("%d/%m/%Y")
  return f"{data_normalizzata} · {orario.strftime('%H:%M')}"


def data_da_display_evento(evento):
  """Estrae la sola data da un valore storico o dal nuovo formato con orario."""
  valore = str(evento.get("data_display") or "").split("·", 1)[0].strip()
  return valore or str(evento.get("data") or date.today().strftime("%d/%m/%Y"))
...
def orario_per_widget(evento):
  valore = orario_evento(evento) or "12:00"
...
if "noleggio_demo_anno" not in str_lit.session_state:
  str_lit.session_state.noleggio_demo_anno = date.today().year
if "tipo_calendario" not in str_lit.session_state:
  str_lit.session_state.tipo_calendario = "noleggi"
if "noleggio_demo_eliminazione_in_attesa" not in str_lit.session_state:
  str_lit.session_state.noleggio_demo_eliminazione_in_attesa = None
if "notifiche_demo" not in str_lit.session_state:
  str_lit.session_state.notifiche_demo = []
if "notifiche_da_mostrare" not in str_lit.session_state:
  str_lit.session_state.notifiche_da_mostrare = []
if "notifiche_lette_per_ruolo" not in str_lit.session_state:
  str_lit.session_state.notifiche_lette_per_ruolo = {}
...
def modale_modifica_noleggio_demo(noleggio_id):
  ...
  if str_lit.session_state.get("noleggio_demo_eliminazione_in_attesa") == noleggio_id:
    ...
    with conferma:
      if str_lit.button("Conferma eliminazione", key=f"conferma_elimina_noleggio_{noleggio_id}", type="primary", use_container_width=True):
        ...
        str_lit.rerun()
    with annulla:
      if str_lit.button("Annulla", key=f"annulla_elimina_noleggio_{noleggio_id}", use_container_width=True):
        str_lit.session_state.noleggio_demo_eliminazione_in_attesa = None
        str_lit.rerun()
    return
  with str_lit.form(f"form_modifica_noleggio_demo_{noleggio_id}"):
    ...
    col_a, col_b = str_lit.columns(2)
    with col_a:
      data_inizio = str_lit.date_input("Data inizio", noleggio["inizio"].date(), format="DD/MM/YYYY", disabled=not puo_modificare)
    ...
    with col_b:
      data_fine = str_lit.date_input("Data fine", noleggio["fine"].date(), format="DD/MM/YYYY", disabled=not puo_modificare)
...
def modale_catering_da_calendario(evento):
  ...
  with col1:
    str_lit.markdown(f"**Data:** {data_da_display_evento(evento)}")
    str_lit.markdown(f"**Location:** {evento.get('location', '-')}")
  with col2:
    str_lit.markdown(f"**Orario:** {orario_evento(evento) or 'Non indicato'}")
    str_lit.markdown(f"**Ospiti:** {evento.get('ospiti', 0)}")
  with col3:
  ...
  if ruolo in {"Amministratore", "Wedding"}:
    with str_lit.popover("🗑️ Elimina evento Catering"):
      ...
      if str_lit.button("Conferma eliminazione evento", key=f"elimina_catering_cal_{evento.get('id', evento.get('nome_evento', 'evento'))}", type="primary", use_container_width=True):
        try:
          if evento.get("id"):
            supabase.table("eventi_catering").delete().eq("id", evento.get("id")).execute()
          else:
            supabase.table("eventi_catering").delete().eq("nome_evento", evento.get("nome_evento")).eq("data", evento.get("data")).execute()
        except Exception as errore:
          str_lit.error(f"Impossibile eliminare l'evento: {errore}")
        str_lit.session_state.eventi_catering = [
            ev for ev in str_lit.session_state.eventi_catering
            if not ((evento.get("id") and ev.get("id") == evento.get("id")) or
                    (not evento.get("id") and ev.get("nome_evento") == evento.get("nome_evento") and ev.get("data") == evento.get("data")))
        ]
        str_lit.rerun()


def mostra_noleggi_demo():
  # Gli eventi Catering non erano caricati in questa schermata: il calendario
  # mostrava quindi solo il vecchio evento demo e apriva dettagli errati.
  ...
  # Magazzino 1 e Amministratore possono vedere anche il Catering nel calendario Noleggi.
  ...
  str_lit.caption("Visualizza solo gli eventi Catering del reparto." if solo_catering else "Visualizza gli eventi Noleggio del calendario.")
  ...
  .noleggi-day-muted {height:86px; background:#f8fafc; border:1px solid #eef2f6; border-radius:8px;}
  .noleggi-markers {display:flex; flex-wrap:wrap; justify-content:center; gap:3px; min-height:28px; padding:5px 1px 1px;}
  .noleggi-marker {display:inline-block; border-radius:999px; padding:3px 5px; color:white; font-size:.55rem; font-weight:800; line-height:1; letter-spacing:.03em; white-space:nowrap;}
  .noleggi-event-bar {width:100%; overflow:hidden; text-overflow:ellipsis; border-radius:4px; padding:3px 5px; margin:2px 0; color:white; font-size:.57rem; font-weight:800; line-height:1.15; white-space:nowrap; text-align:left;}
  .noleggi-event-bar-rental {background:#0056b3;}
  .noleggi-event-bar-pending {background:#eab308; color:#3b2f00;}
  .noleggi-event-bar-catering {background:#166534;}
  .noleggi-event-bar-start {border-top-left-radius:6px; border-bottom-left-radius:6px; margin-left:-5px; width:calc(100% + 5px);}
  .noleggi-event-bar-end {border-top-right-radius:6px; border-bottom-right-radius:6px; margin-right:-5px; width:calc(100% + 5px);}
  .noleggi-event-bar:not(.noleggi-event-bar-start):not(.noleggi-event-bar-end) {border-radius:0; margin-left:-5px; margin-right:-5px; width:calc(100% + 10px);}
  .noleggi-day-empty {font-size:.72rem; color:#98a2b3; padding-top:5px;}
  </style>
  """, unsafe_allow_html=True)
  ...
  eventi_catering_calendario = [
      evento_catering
      for evento_catering in str_lit.session_state.get("eventi_catering", [])
      if mostra_catering and evento_catering.get("nome_evento")
  ]
  ...
  with calendario_col:
    ...
    str_lit.markdown(f"<div class='noleggi-month-note'>Seleziona un giorno per aprire direttamente gli eventi presenti.</div>", unsafe_allow_html=True)
    legenda = []
    if mostra_noleggi:
      legenda.extend(["<span><i class='noleggi-dot' style='background:#0056b3'></i>Noleggio confermato</span>", "<span><i class='noleggi-dot' style='background:#eab308'></i>Noleggio non confermato</span>"])
    if mostra_catering:
      legenda.append("<span><i class='noleggi-dot' style='background:#166534'></i>Evento Catering</span>")
    str_lit.markdown(f"<div class='noleggi-legend'>{''.join(legenda)}</div>", unsafe_allow_html=True)
        ...
        with colonna:
          ...
          with str_lit.container(border=True):
            ...
            if str_lit.button(etichetta, key=f"demo_giorno_{giorno}", use_container_width=True):
              ...
              str_lit.session_state.noleggio_demo_modifica = None
              str_lit.session_state.evento_catering_demo_selezionato = None
              eventi_del_giorno = list(eventi_giorno) + list(catering_giorno)
              if len(eventi_del_giorno) == 1:
                evento_unico = eventi_del_giorno[0]
                if evento_unico in catering_giorno:
                  str_lit.session_state.evento_catering_demo_selezionato = evento_unico
                elif not (evento_unico.get("stato") != "confermato" and ruolo_calendario == "Magazzino"):
                  str_lit.session_state.noleggio_demo_modifica = evento_unico.get("id")
              str_lit.rerun()
            marker_html = []
  ...
  with pannello_col:
    ...
    with str_lit.container(border=True):
      ...
      str_lit.markdown("<div class='noleggi-side-subtitle'>Solo eventi Catering presenti nella giornata selezionata</div>" if solo_catering else "<div class='noleggi-side-subtitle'>Eventi Catering e noleggi presenti nella giornata selezionata</div>", unsafe_allow_html=True)
      if puo_creare_noleggio and str_lit.button("＋ Crea nuovo noleggio", type="primary", use_container_width=True, key="demo_crea_noleggio_laterale"):
      ...
      if not eventi_selezionati and not catering_selezionati:
        ...
        for indice_catering, evento_catering in enumerate(catering_selezionati):
          if str_lit.button(f"🟩 Catering — {evento_catering.get('nome_evento', 'Evento Catering')}", key=f"cal_catering_{giorno_selezionato}_{indice_catering}", use_container_width=True):
            str_lit.session_state.evento_catering_demo_selezionato = evento_catering
            str_lit.session_state.noleggio_demo_modifica = None
            str_lit.rerun()
          dettagli_orario = orario_evento(evento_catering) or "orario non indicato"
          str_lit.caption(f"{dettagli_orario} · {evento_catering.get('location', 'Location non indicata')} · {evento_catering.get('ospiti', 0)} ospiti")
        for noleggio in eventi_selezionati:
          ruolo_corrente = (str_lit.session_state.get("utente_loggato") or {}).get("ruolo", "")
          non_confermato_non_apribile = noleggio.get("stato") != "confermato" and ruolo_corrente == "Magazzino"
          ...
          if str_lit.button(f"{colore} {noleggio['titolo']}\n{orario}", key=f"demo_pannello_noleggio_{noleggio['id']}", use_container_width=True, disabled=non_confermato_non_apribile):
            str_lit.session_state.noleggio_demo_modifica = noleggio["id"]
            str_lit.session_state.noleggio_demo_crea_data = None
            str_lit.session_state.evento_catering_demo_selezionato = None
            str_lit.rerun()
          if non_confermato_non_apribile:
            str_lit.caption("Disponibile dopo la conferma del noleggio.")
          else:
  ...
  if evento_da_aprire:
    # L'azzeramento prima della modale impedisce che il dettaglio Catering
    # venga riaperto quando si seleziona un altro giorno del calendario.
    str_lit.session_state.evento_catering_demo_selezionato = None
    ...
    if str_lit.form_submit_button(
      ...
      if not n_nome.strip():
        ...
        nuovo_ev = {
            ...
            "data_display": data_display_con_orario(n_data, n_orario),
            "location": n_loc,
            "ospiti": n_ospiti,
            "bambini": n_bambini,
            "staff": n_staff,
        ...
        destinatari_nuovo_evento = ["Sala", "Cucina", "Magazzino"]
        str_lit.session_state.notifiche_demo.append({"tipo": "catering", "destinatari": destinatari_nuovo_evento, "testo": f"Nuovo evento Catering creato: {n_nome}."})
        if t_tutti.strip():
          str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": destinatari_nuovo_evento, "testo": f"Nuova nota per tutti nell’evento {n_nome}."})
        if t_sala.strip():
          str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Sala"], "testo": f"Nuova nota per la Sala nell’evento {n_nome}."})
        if t_cucina.strip():
          str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Cucina"], "testo": f"Nuova nota per la Cucina nell’evento {n_nome}."})
        if t_mag.strip():
          str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Magazzino"], "testo": f"Nuova nota per il Magazzino nell’evento {n_nome}."})
...
def modale_modifica_evento(idx_ev):
  ...
  m_data_precompilata = data_da_display_evento(ev_mod)
  with str_lit.form(f"form_mod_ev_dialog_{idx_ev}"):
    ...
    if btn_salva_ev:
      ...
      ev_mod.update({
          "nome_evento": m_nome,
          "data": dt_parsed.strftime("%Y-%m-%d"),
          "data_display": data_display_con_orario(m_data, m_orario),
          "location": m_loc,
          "ospiti": m_osp,
          "bambini": m_bam,
          "staff": m_stf,
      ...
      if mt_tutti.strip():
        str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Sala", "Cucina", "Magazzino"], "testo": f"Nuova nota per tutti nell’evento {m_nome}."})
      if mt_sala.strip():
        str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Sala"], "testo": f"Nuova nota per la Sala nell’evento {m_nome}."})
      if mt_cucina.strip():
        str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Cucina"], "testo": f"Nuova nota per la Cucina nell’evento {m_nome}."})
      if mt_mag.strip():
        str_lit.session_state.notifiche_demo.append({"tipo": "nota", "destinatari": ["Magazzino"], "testo": f"Nuova nota per il Magazzino nell’evento {m_nome}."})
      ...
      with c2:
        with str_lit.container(border=True):
          ...
          if str_lit.button(
            ...
            str_lit.session_state.area_selezionata = "opzione_noleggi"
            str_lit.session_state.tipo_calendario = "catering"
            str_lit.rerun()
            str_lit.stop()
        ...
        if is_magazzino or is_magazzino2:
          with str_lit.container(border=True):
            ...
            if str_lit.button("Apri Noleggi", use_container_width=True, type="primary", key="btn_h_noleggi_magazzino2_vicino"):
              ...
              str_lit.session_state.noleggio_demo_giorno_selezionato = date.today()
              str_lit.rerun()
        elif is_cucina or is_sala:
          with str_lit.container(border=True):
            if logo_catering_b64:
              str_lit.markdown(f'<div style="height:100px;display:flex;align-items:center;justify-content:center;margin-bottom:15px;"><img src="data:image/png;base64,{logo_catering_b64}" style="height:120px;width:100%;object-fit:contain;"></div>', unsafe_allow_html=True)
            else:
              str_lit.markdown("<div style='height:100px;display:flex;align-items:center;justify-content:center;font-size:3rem;margin-bottom:15px;'>📅</div>", unsafe_allow_html=True)
            str_lit.markdown("### Calendario Catering")
            str_lit.markdown("<div class='card-desc'>Visualizza solo gli eventi Catering del tuo reparto.</div>", unsafe_allow_html=True)
            if str_lit.button("Apri Calendario Catering", use_container_width=True, type="primary", key="btn_h_calendario_catering_reparto"):
              str_lit.session_state.area_selezionata = "opzione_noleggi"
              str_lit.session_state.tipo_calendario = "catering"
              str_lit.session_state.noleggio_demo_mese = date.today().month
              str_lit.session_state.noleggio_demo_anno = date.today().year
