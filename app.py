...
def modale_crea_noleggio_demo(data_selezionata):
  ...
  with str_lit.form("form_crea_noleggio_demo"):
    ...
    with a1:
      bolla = str_lit.file_uploader("Bolla", type=["pdf", "jpg", "jpeg", "png"])
    with a2:
...
def modale_modifica_noleggio_demo(noleggio_id):
  ...
  with str_lit.form(f"form_modifica_noleggio_demo_{noleggio_id}"):
    ...
    if ruolo_corrente in {"Amministratore", "Magazzino2"}:
      str_lit.write(f"Bolla: `{noleggio['bolla']}`")
...
def mostra_noleggi_demo():
  ...
  with col_anno:
    anno = str_lit.number_input("Anno calendario", min_value=2025, max_value=2035, value=str_lit.session_state.noleggio_demo_anno, step=1, key="noleggio_anno_select")
